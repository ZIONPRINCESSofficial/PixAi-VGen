  ...
  const generate = async () => {
    ...
    try {
      if (mode === "text-to-image" || mode === "image-to-image") {
        ...
        // Image to image — use uploaded image as reference
        const basePrompt = mode === "image-to-image" 
          ? `${prompt || "enhance and improve this portrait"}, photorealistic portrait enhancement, clean skin, perfect lighting, professional headshot, ${qualityPrompt}`
        ...
        const imageUrl = `https://image.pollinations.ai/prompt/${fullPrompt}?width=${w}&height=${h}&seed=${seed}&nologo=true&enhance=true&model=flux`;

        clearInterval(interval);
        setSteps(100);
        setTimeout(() => {
          setGenerating(false);
          setGenerated({ prompt: basePrompt, style, ratio, resolution, duration, imageUrl, type: mode, isVideo: false });
        }, 500);
    ...
    <div style={{ padding:28, overflowY:"auto", flex:1 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 380px", gap:24 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          {/* Generate button */}
          <button className="fade-up s4 glow-btn" onClick={generate} disabled={generating} style={{ padding:"16px 32px", background:generating?"var(--card)":"linear-gradient(135deg,var(--accent),var(--accent2))", color:generating?"var(--muted)":"#fff", borderRadius:12, fontWeight:700, fontSize:15, transition:"all .3s", display:"flex", alignItems:"center", justifyContent:"center", gap:10, border:`1px solid ${generating?"var(--border)":"transparent"}` }}>
            {generating ? (
              <>
                <div style={{ width:16, height:16, border:"2px solid var(--muted)", borderTopColor:"var(--accent)", borderRadius:"50%", animation:"spin 1s linear infinite" }} />
                Generating... {steps}%
              </>
            ) : (
              <>✦ Generate {mode==="text-to-image"?"Image":mode==="text-to-video"?"Video":"Video from Image"}</>
            )}
          </button>
          ...
          <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:20, flex:1 }}>
            ...
            {generated ? (
              <div style={{ animation:"fadeUp .5s ease both" }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <button onClick={() => { if(generated.imageUrl){ const a=document.createElement("a"); a.href=generated.imageUrl; a.download="pixai-image.jpg"; a.target="_blank"; a.click(); } else { alert("⬇️ Downloading your creation..."); }}} style={{ flex:1, padding:"10px", background:"linear-gradient(135deg,var(--accent),var(--accent2))", color:"#fff", borderRadius:8, fontWeight:700, fontSize:12 }}>⬇ Download</button>
